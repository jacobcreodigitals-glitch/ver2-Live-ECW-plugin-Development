document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('.ecw-arc-cards').forEach((section) => {
        ...
        const ctx = gsap.context(() => {
            ...
            const pinHeight = section.querySelector('.ecw-arc-cards_TEMP_pin');
            const container = section.querySelector('.ecw-arc-cards_TEMP_viewport');
            const circlesWrapper = section.querySelector('.ecw-arc-cards_TEMP_track');
            const circles = section.querySelectorAll('.ecw-arc-cards_TEMP_ring');

            // Defensive: exit if required elements are missing
            if (!pinHeight || !container || !circles.length) return;

            // Hide scroll indicator (only if it exists)
            ...
            circles.forEach((circle, index) => {
                ...
                const card = circle.querySelector('.ecw-arc-cards_TEMP_item');
                if (card) {
                    gsap.to(card, {
                        rotation: baseRotation + (angle * index),
                        y: '-50%',
